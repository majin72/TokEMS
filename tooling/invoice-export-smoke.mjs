...
try {
  ...
  const organization = await pool.query(`select id from organizations where slug = $1 limit 1`, [
    'tokems-demo',
  ]);
  assert(organization.rows[0]?.id, 'Seed organization does not exist');
  exportJobId = randomUUID();
  await pool.query(
    `insert into invoice_export_jobs
      (id, organization_id, requested_by, status, filters, row_count)
     select $1, $2, id, 'queued', '{}'::jsonb, 0
     from users where email = $3 limit 1`,
    [exportJobId, organization.rows[0].id, 'admin@tokems.local'],
  );
  await pool.query(
    `insert into outbox_events
      (organization_id, event_type, correlation_id, payload)
     values ($1, 'InvoiceExportRequested', $2, $3::jsonb)`,
    [
  ...
  for (let attempt = 0; attempt < 60; attempt += 1) {
    ...
    job = await request(`/admin/invoices/export-jobs/${exportJobId}`, {
