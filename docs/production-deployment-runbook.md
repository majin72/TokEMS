...
报名表规范必须同时匹配后台已保存表单与当前生效发布中的表单，包括字段、条款正文和版本。存在差异时先通过后台保存正确版本并生效，再导出快照。生产预检的只读观察导出仍允许记录这种差异，供发布流程识别与修复漂移；提交快照的校验保持严格一致。
...
`check` 和普通 `deploy` 会以只读数据库查询核对三类待处理记录：微信支付的进行中或结果未知尝试、尚未结清的支付通知，以及已付款但缺少电子票的订单。旧通知即使停留在失败状态，只要存在同组织、同订单、同微信交易号、同金额和币种的已提交支付及票证，也可被只读核验认定已结清；已退款订单还必须具备同笔支付的成功退款及匹配的退款总额。该判定支持首次上线时的历史记录，API 恢复后再清理对应通知状态。任一计数非零或查询失败时，发布在停止 API/Worker 前退出，当前服务继续处理支付；待补单完成后重新运行原命令。过期时间本身不能证明微信交易已关闭，脚本不会按时间直接忽略支付尝试。

镜像准备完成后、停服前会再次检查。API/Worker 停止后还会复查，覆盖检查与停服之间的新支付。如果此时发现支付活动或无法取得有效证据，脚本确认没有执行数据库操作、迁移身份仍等于原版本，然后恢复原镜像标签并启动原有 API/Worker 容器；健康、构建身份和 Worker 就绪检查通过后归档恢复标记，以失败状态结束本次发布。取消证据记录为 `status=cancelled-payment-activity`，不会继续迁移、重建应用或切换目标版本。恢复过程中失败时保留既有受保护恢复流程。

已有 `RECOVERY_REQUIRED` 的前向修复通过 `--resume-recovery` 进入独立恢复路径，允许在支付尚未确认时修复已经冻结的服务。API 恢复正常数据库写权限后，会在启动时和每 15 秒扫描未完成的微信支付，覆盖尚未过期的订单；查单与顾客页面共用查询节流和数据库条件更新，准备下单和关闭订单的有效租约保持受保护。仅切换收款开关且其余配置、有效密钥均未变化时保留已有验证，关闭新支付不影响已有交易的查单、关单和回调验签。修改密钥或商户配置后必须重新通过微信连接验证；暂停收款期间也可以验证配置，恢复流程始终只信任已验证的凭据。原商户绑定与当前配置不一致时继续阻断并要求核验。查单成功先持久化到通知收件箱，再走同一条幂等确认、出票和重试流程。三个下单通道的迟到响应只允许更新本次准备租约，不能覆盖已付款、已退款或正在关闭的记录；无法确认的下单结果保留为未知状态，继续查单。确认重试耗尽时保留原交易状态，并向组织管理员/财务排队发送通知、记录审计事件；管理员需核对交易、补票或退款，确认成功的支付与票证可以自动结清对应的旧异常通知。

对已经超过本地及预支付窗口 60 秒的孤儿尝试，只有使用原商户查询、微信响应签名通过且返回 HTTP 404 / `ORDER_NOT_EXIST`，并成功取得状态租约时，才允许标记关闭并记录审计。网络超时、未签名错误、商户不匹配和没有原商户绑定的历史记录继续保留，运维必须核实原始微信交易，不能直接改为付款成功或删除记录。错误码依据：[微信支付查单文档](https://pay.wechatpay.cn/doc/v3/merchant/4012791837)。

当前发布仍包含备份、迁移、只读验收和恢复写入的维护窗口。支付检查负责避开进行中的交易并在竞态发生时撤销发布。全程连续提供服务需要另外实施新旧实例并存、流量切换和兼容性迁移；不能通过移除写冻结来获得同等数据保护。上线本修复前，先安排无待处理支付的窗口；这次发布完成后，新的主动补单机制才开始生效。首版若遇到旧 API 无法结清的孤儿支付或缺票交易，应保留服务并先核验、修复遗留记录，禁止借用 `--resume-recovery` 绕过普通发布门禁。持续有新付款尝试时发布可能持续延期；已安装本修复后，可关闭新支付并等待已有尝试结清，再运行原发布命令。
...
规范数据核验进程的虚拟内存上限为 512 MiB。Alibaba Linux 的系统 Python 3.6 启动时可能已经保留约 226 MiB 虚拟地址空间；内存预算须包含解释器与规范素材 JSON 的解码开销。预检会在写冻结前使用相同预算解析经过证明的目标快照，解析失败会阻断本次发布。物理可用内存与进程虚拟地址上限需分别检查。
